...
def sheets_setup():
    ...
    try:
        token = get_sheets_token()
        # Leer A1
        url = f"https://sheets.googleapis.com/v4/spreadsheets/{SHEET_ID}/values/Hoja1!A1"
        res = requests.get(url, headers={"Authorization": f"Bearer {token}"})
        data = res.json()
        if "values" not in data:
            # Escribir headers
            headers_url = f"https://sheets.googleapis.com/v4/spreadsheets/{SHEET_ID}/values/Hoja1!A1:N1"
