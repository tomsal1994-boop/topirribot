...
def msg_depto(p):
    pf = f"USD {p['precio']:,}".replace(",",".") if p.get("precio",0) > 0 else "Consultar precio"
    zona = p.get("zona","palermo")
    if zona == "botanico": zona_emoji = f"🌿 {p.get('calle','Zona Botánico')}"
    elif zona == "recoleta": zona_emoji = "🏛 Recoleta"
    else: zona_emoji = "🌳 Palermo Chico"
    return (f"🏠 <b>{p['titulo'][:70]}</b>\n💰 {pf} | 🛏 {p['ambientes']} amb\n"
            f"✅ {p.get('motivo','Nueva publicación')}\n📍 {zona_emoji} — {p['fuente']}\n"
...
def scrape_botanico():
    """Busca departamentos en calles específicas frente al Botánico"""
    res = []
    calles_zp = [
        ("republica-arabe-siria", "Rep. Árabe Siria"),
        ("arenales", "Arenales"),
        ("juncal", "Juncal"),
        ("beruti", "Beruti"),
        ("republica-de-la-india", "Rep. de la India"),
    ]
    for calle_slug, calle_nombre in calles_zp:
        for portal, url in [
            ("Zonaprop", f"https://www.zonaprop.com.ar/departamentos-venta-{calle_slug}-palermo.html"),
            ("Argenprop", f"https://www.argenprop.com/departamentos/venta/calle-{calle_slug}"),
        ]:
            try:
                r = requests.get(url, headers=HEADERS_HTTP, timeout=15)
                soup = BeautifulSoup(r.text, "html.parser")
                if portal == "Zonaprop":
                    for card in soup.select("[data-id]"):
                        ...
                        le = card.select_one("a[href]"); href = le["href"] if le else ""
                        link = f"https://www.zonaprop.com.ar{href}" if href.startswith("/") else href
                        te = card.select_one(".postingCardTitle,h2,.title"); titulo = te.get_text(strip=True) if te else f"Depto {calle_nombre}"
                        de = card.select_one(".postingCardDescription,.description"); desc = de.get_text(strip=True)[:300] if de else ""
                        se = card.select_one("[data-surface]"); sup = se.get("data-surface","") if se else ""
                        if PRECIO_MIN <= precio <= PRECIO_MAX:
                            res.append({"id":f"bot_zp_{pid}","fuente":"Zonaprop","titulo":titulo,"precio":precio,"link":link,"ambientes":"?","descripcion":desc,"superficie":sup,"zona":"botanico","calle":calle_nombre})
                else:
                    for card in soup.select(".listing__item,[class*='listing-item']"):
                        le = card.select_one("a[href]")
                        if not le: continue
                        href = le["href"]; pid = hashlib.md5((calle_slug+href).encode()).hexdigest()[:12]
                        link = f"https://www.argenprop.com{href}" if href.startswith("/") else href
                        pe = card.select_one("[class*='price'],.price"); pt = pe.get_text(strip=True) if pe else ""
                        precio = 0
                        for p in pt.replace(".","").replace(",","").split():
                            if p.isdigit() and len(p)>=5: precio=int(p); break
                        te = card.select_one("h2,h3,[class*='title']"); titulo = te.get_text(strip=True) if te else f"Depto {calle_nombre}"
                        de = card.select_one("[class*='description'],p"); desc = de.get_text(strip=True)[:300] if de else ""
                        if PRECIO_MIN <= precio <= PRECIO_MAX:
                            res.append({"id":f"bot_ap_{pid}","fuente":"Argenprop","titulo":titulo,"precio":precio,"link":link,"ambientes":"?","descripcion":desc,"superficie":"","zona":"botanico","calle":calle_nombre})
            except Exception as e: log.warning(f"Botánico {calle_nombre} {portal}: {e}")

    for calle_ml in ["Republica-Arabe-Siria","Arenales","Juncal","Beruti","Republica-de-la-India"]:
        try:
            url = f"https://listado.mercadolibre.com.ar/inmuebles/departamentos/venta/{calle_ml.lower()}-palermo_NoIndex_True"
            r = requests.get(url, headers=HEADERS_HTTP, timeout=15)
            soup = BeautifulSoup(r.text, "html.parser")
            for card in soup.select(".ui-search-result__wrapper,[class*='poly-card']"):
                le = card.select_one("a[href]")
                if not le: continue
                href = le["href"]; pid = hashlib.md5(href.encode()).hexdigest()[:12]
                te = card.select_one("h2,.poly-component__title,[class*='title']"); titulo = te.get_text(strip=True) if te else f"Depto {calle_ml}"
                pe = card.select_one(".andes-money-amount__fraction,[class*='price']"); pt = pe.get_text(strip=True) if pe else "0"
                precio = int(pt.replace(".","").replace(",","").strip() or 0)
                if PRECIO_MIN <= precio <= PRECIO_MAX:
                    res.append({"id":f"bot_ml_{pid}","fuente":"MercadoLibre","titulo":titulo,"precio":precio,"link":href,"ambientes":"?","descripcion":"","superficie":"","zona":"botanico","calle":calle_ml.replace("-"," ")})
        except Exception as e: log.warning(f"ML Botánico {calle_ml}: {e}")
    return res
        ...
        for upd in updates:
            ...
            if texto.startswith("/start") or texto.startswith("/ayuda"):
                ...
                try:
                    ...
                    send(chat_id, f"⚠️ Error: {e}")
            elif texto.startswith("/botanico"):
                send(chat_id, "🌿 Buscando en Rep. Árabe Siria, Arenales, Juncal, Beruti y Rep. de la India...")
                try:
                    import time
                    props = scrape_botanico()
                    seen = load_seen(); nuevas = []
                    for p in props:
                        if p["id"] not in seen:
                            seen.add(p["id"])
                            p["motivo"] = f"📍 {p.get('calle','Zona Botánico')}"
                            nuevas.append(p)
                    save_seen(seen)
                    if not nuevas: send(chat_id, "😴 Sin publicaciones nuevas en esas calles.")
                    else:
                        send(chat_id, f"🌿 <b>{len(nuevas)} publicación(es) nueva(s) zona Botánico</b>")
                        for p in nuevas[:8]: send(chat_id, msg_depto(p)); time.sleep(0.5)
                except Exception as e: log.error(f"botanico: {e}"); send(chat_id, f"⚠️ Error: {e}")
